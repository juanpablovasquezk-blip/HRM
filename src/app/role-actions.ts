...
export async function getUserRole() {
  ...
  const metaRole = (user.user_metadata?.role || '').toUpperCase().trim();
  ...
  console.log(`[AUTH] getUserRole for ${user.email}: ${finalRole} (Metadata: ${user.user_metadata?.role}, DB: ${dbUser?.role})`);
  return finalRole;
}
