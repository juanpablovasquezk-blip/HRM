...
export async function GET(request: Request) {
  ...
  try {
    ...
    // 5. Build formatted date for the email
    ...
    if (absenceCount === 0) {
      // ---- No absences: send positive confirmation ----
      emailHtml = `
        <!DOCTYPE html>
        <html>
        <head><meta charset="utf-8"><title>Reporte de Ausencias</title></head>
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f8fafc; margin: 0; padding: 40px 20px; color: #334155;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; border: 1px solid #e2e8f0; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.05);">
            <div style="background: linear-gradient(135deg, #16a34a 0%, #22c55e 100%); padding: 32px 24px; text-align: center;">
              <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 800;">Reporte Diario de Ausencias</h1>
              <p style="color: #dcfce7; margin: 8px 0 0 0; font-size: 14px; font-weight: 500;">${formattedDate}</p>
            </div>
            <div style="padding: 48px 24px; text-align: center;">
              <div style="font-size: 64px; margin-bottom: 16px;">&#9989;</div>
              <h2 style="color: #16a34a; margin: 0 0 8px 0; font-size: 22px; font-weight: 700;">Sin inasistencias</h2>
              <p style="color: #64748b; margin: 0; font-size: 15px; line-height: 1.6;">
                No se reportaron inasistencias el d&iacute;a <strong>${formattedDate}</strong>.<br/>
                Toda la dotaci&oacute;n planificada se present&oacute; correctamente.
              </p>
            </div>
            <div style="background-color: #f8fafc; padding: 20px; text-align: center; border-top: 1px solid #e2e8f0; font-size: 11px; color: #94a3b8;">
              <p style="margin: 0;">Este es un correo autom&aacute;tico generado por HRM Roster Manager.</p>
              <p style="margin: 4px 0 0 0;">Minerquim Log&iacute;stica Aeropuerto &copy; ${new Date().getFullYear()}</p>
            </div>
          </div>
        </body>
        </html>
      ...
      // ---- Has absences: build the table ----

      // 6. Fetch names of supervisors who updated the attendance
          ...
          <div style="max-width: 800px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; border: 1px solid #e2e8f0; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.05);">
            ...
            <div style="padding: 32px 24px;">
              <p style="margin: 0 0 20px 0; font-size: 15px; line-height: 1.6; color: #475569;">
                Se reportaron las siguientes inasistencias el d&iacute;a <strong>${formattedDate}</strong>:
              </p>
                ...
                <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 13px;">
                  <thead>
                    <tr style="background-color: #f1f5f9; border-bottom: 2px solid #e2e8f0;">
                      ...
                      <th style="padding: 12px; font-weight: 800; text-transform: uppercase; color: #475569;">&Aacute;rea / Cargo</th>
                      ...
                      <th style="padding: 12px; font-weight: 800; text-transform: uppercase; background-color: #fee2e2; color: #991b1b;">Observaci&oacute;n</th>
                    </tr>
                  </thead>
                  <tbody>
                    ${tableRows}
    ...
    // 8. Send the email via SMTP (Nodemailer)
    ...
    const info = await transporter.sendMail({
      ...
      subject: absenceCount > 0
        ? `[Ausencias] Reporte de Inasistencias - ${targetDateStr}`
        : `[Ausencias] ✅ Sin inasistencias - ${targetDateStr}`,
      html: emailHtml,
    });
