...
export interface AuthorizationPDFParams {
  workerName: string;
  workerRut: string;
  workerEmail: string;
  companyName: string;
...
export function formatAuthorizationFileName(workerName: string, workerRut: string): string {
  ...
  const cleanRut = workerRut.trim().replace(/[^0-9kK-]/g, '');

  return `AUTORIZACION_RIOHS_${cleanName}_${cleanRut}.pdf`;
}

export async function createAuthorizationPDFDoc(params: AuthorizationPDFParams): Promise<jsPDF> {
  const { workerName, workerRut, workerEmail, companyName, companyRut } = params;
  ...
  doc.text('AUTORIZACIÓN ENVÍO INFORMACIÓN MEDIOS DIGITALES', pageW / 2, currentY, { align: 'center' });
...
export async function generateAuthorizationPDF(params: AuthorizationPDFParams) {
  const doc = await createAuthorizationPDFDoc(params);
  const fileName = formatAuthorizationFileName(params.workerName, params.workerRut);
  doc.save(fileName);
}
