...
export function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      {/* Settings & Mobile App Links */}
      <div className="px-3 py-2 space-y-1">
        <Link
          href="/supervisor"
          className={cn(
            'flex items-center gap-3 rounded-xl px-3 py-2 text-xs font-bold text-orange-600 bg-orange-50/80 hover:bg-orange-100 hover:text-orange-700 dark:bg-orange-950/40 dark:text-orange-400 dark:hover:bg-orange-900/40 transition-all duration-200 border border-orange-200/60 dark:border-orange-900/40',
            collapsed && 'justify-center px-2'
          )}
          title="App Móvil (Supervisor / Terreno)"
        >
          <Smartphone className="h-4 w-4 shrink-0" />
          {!collapsed && <span>App Móvil (Terreno)</span>}
        </Link>
        ...
        {(role === 'ADMIN' || role === 'HR') && (
          <Link
            href="/settings"
            className={cn(
              'flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-100 dark:hover:bg-slate-800/50 transition-all duration-200',
              collapsed && 'justify-center px-2'
            )}
          >
